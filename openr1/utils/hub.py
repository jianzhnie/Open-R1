...
def push_to_hub_revision(training_args: SFTConfig | GRPOConfig,
    ...
    repo_url = create_repo(repo_id=training_args.hub_model_id,
                           private=True,
                           exist_ok=True)
    ...
    logger.info(
        f'Pushing to the Hub revision {training_args.hub_model_revision}...')
...
def check_hub_revision_exists(training_args: SFTConfig | GRPOConfig):
    ...
    if repo_exists(training_args.hub_model_id):
        if training_args.push_to_hub_revision is True:
            ...
            revisions = [
                rev.name
                for rev in list_repo_refs(training_args.hub_model_id).branches
            ]
...
def get_param_count_from_repo_id(repo_id: str) -> int:
    """Function to get model param counts from safetensors metadata or find
    patterns like 42m, 1.5b, 0.5m or products like 8x7b in a repo ID."""
    try:
...
def get_gpu_count_for_vllm(model_name: str,
                           revision: str = 'main',
                           num_gpus: int = 8) -> int:
    """vLLM enforces a constraint that the number of attention heads must be
    divisible by the number of GPUs and 64 must be divisible by the number of
    GPUs.

    This function calculates the number of GPUs to use for decoding based on
