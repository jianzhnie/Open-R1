...
from openr1.utils.import_utils import is_e2b_available
...
def accuracy_reward(completions, solution, **kwargs):
    """Reward function that checks if the completion is the same as the ground
    truth."""
...
def reasoning_steps_reward(completions, **kwargs):
    ...
    matches = [
        len(re.findall(pattern, content)) for content in completion_contents
    ]
...
def len_reward(completions: list[Dict[str, str]], solution: list[str],
               **kwargs) -> float:
...
def get_repetition_penalty_reward(ngram_size: int, max_penalty: float):
    ...
    def repetition_penalty_reward(completions, **kwargs) -> float:
        """reward function the penalizes repetitions ref implementation:
        https://github.com/eddycmu/demystify-long-
        cot/blob/release/openrlhf/openrlhf/reward/repetition.py.
...
def code_reward(completions, **kwargs) -> list[float]:
    ...
    try:
        ...
        """
        code_snippets = [
            extract_code(completion[-1]['content'])
