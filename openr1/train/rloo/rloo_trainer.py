...
from trl.trainer.utils import (
    ...
    get_reward, log_table_to_comet_experiment, prepare_deepspeed,
    print_rich_table, selective_log_softmax, truncate_response)

from openr1.train.rloo.rloo_config import RLOOConfig
        ...
        if self.is_deepspeed_enabled:
            if isinstance(self.reward_model, nn.Module):
                self.reward_model = prepare_deepspeed(
                    self.reward_model, args.per_device_train_batch_size,
                    args.fp16, args.bf16)
            self.ref_policy = prepare_deepspeed(
                self.ref_policy, args.per_device_train_batch_size, args.fp16,
                args.bf16)
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0],
                    ...
                    if args.stop_token_id is not None:  # handle the edge case when stop_token_id exists but is 0
                        postprocessed_response = truncate_response(
                            args.stop_token_id, processing_class.pad_token_id,
                            response)
                ...
                if args.normalize_reward:
                    ...
                    scores = torch.clamp(scores, -args.reward_clip_range,
                                         args.reward_clip_range)
                ...
                if args.token_level_kl:
                    ...
                    eos_indices = padding_mask.size(1) - 1 - padding_mask.long(
                    ).fliplr().argmax(dim=1, keepdim=True)
                    last_reward = torch.zeros_like(kl)
                    # Ensure scores has correct shape and type
                    scores_shaped = scores.reshape(-1, 1).to(kl.dtype)
                    last_reward.scatter_(dim=1,
                                         index=eos_indices,
                                         src=scores_shaped)

                    # Combine KL reward and last reward
                    non_score_reward = kl_reward.sum(
                        1)  # Keep this for logging
                ...
                baseline = (rlhf_reward.sum(0) - rlhf_reward) / (args.rloo_k -
                                                                 1)
                ...
                for mini_batch_start in range(0, args.local_batch_size,
                    ...
                    for micro_batch_start in range(
                        ...
                        with accelerator.accumulate(model):
                            ...
                            # Forward pass
                            output = forward(model, mb_query_responses,
                                             processing_class.pad_token_id)
                            ...
                            with torch.no_grad():
                                pg_clipfrac = (pg_losses2 >
                                               pg_losses).float().mean()
                                ...
                                ratio_stats[
                                    ppo_epoch_idx, minibatch_idx,
                                    gradient_accumulation_idx] = new_ratio.mean(
                                    )
            ...
            with torch.no_grad():
                ...
                self.state.epoch = self.state.episode / (
                    args.rloo_k * self.train_dataset_len)  # used by self.log
                self.log(metrics)
