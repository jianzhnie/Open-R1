...
class RepeatRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        indexes = torch.randperm(self.num_samples,
...
class GRPOTrainer(Trainer):
    """Trainer for the Group Relative Policy Optimization (GRPO) method. This
    Algorithm was initially proposed in the paper [DeepSeekMath: Pushing the
    ...
    def __init__(
        ...
        for i, reward_func in enumerate(reward_funcs):
            if isinstance(reward_func, str):
                reward_funcs[
                    i] = AutoModelForSequenceClassification.from_pretrained(
        ...
        possible_values = [
            n_gen for n_gen in range(2, global_batch_size + 1)
            ...
            if self.accelerator.is_main_process:
                ...
                with world_size_patch, profiling_patch, new_group_patch:
                    self.llm = LLM(
                        ...
                        device=vllm_device,
                        gpu_memory_utilization=self.args.
        ...
        if args.sync_ref_model:
            self.add_callback(
                SyncRefModelCallback(ref_model=self.ref_model,
    ...
    def _get_train_sampler(self) -> Sampler:
        ...
        #                                          ...
        effective_batch_size = (self.args.per_device_train_batch_size *
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel,
                                       args: GRPOConfig) -> PreTrainedModel:
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask,
        ...
        logits = logits[:, :
                        -1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
                ...
                if PartialState().default_device.type == 'npu':
                    ...
                    for k, v in state_dict.items():
                        if isinstance(v, torch.Tensor):
                            state_dict[k] = v.to('cpu').to(
                                self.args.vllm_device)
        ...
        if mode == 'train':
            if self.state.global_step % self.num_iterations == 0:
                ...
                self._buffered_inputs[
                    self._step %
                    self.args.gradient_accumulation_steps] = inputs
            else:
    ...
    def _generate_and_score_completions(
        ...
        prompt_ids, prompt_mask = prompt_inputs['input_ids'], prompt_inputs[
            'attention_mask']
        ...
        if self.args.use_vllm:
            ...
            prompt_completion_ids = torch.cat([prompt_ids, completion_ids],
                                              dim=1)
        else:
            # Regular generation path
            with unwrap_model_for_generation(
                    self.model, self.accelerator) as unwrapped_model:
        ...
        eos_idx = torch.full((is_eos.size(0), ),
                             is_eos.size(1),
                             dtype=torch.long,
                             device=device)
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(
            dim=1)]
        ...
        logits_to_keep = completion_ids.size(
            1)  # we only need to compute the logits for the completion tokens
        ...
        rewards_per_func = torch.zeros(len(prompts),
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(
                ...
                output_reward_func = reward_func(prompts=prompts,
    ...
    def compute_loss(self,
        ...
        completion_ids, completion_mask = inputs['completion_ids'], inputs[
            'completion_mask']
        ...
        old_per_token_logps = inputs[
            'old_per_token_logps'] if self.num_iterations > 1 else per_token_logps.detach(
            )
    ...
    def log(self,
        ...
        logs = {**logs, **metrics}
        if version.parse(
                transformers.__version__) >= version.parse('4.47.0.dev0'):
